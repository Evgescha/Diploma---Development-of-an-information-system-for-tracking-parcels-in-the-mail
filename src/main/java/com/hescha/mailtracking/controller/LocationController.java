...
@RequestMapping(LocationController.CURRENT_ADDRESS)
public class LocationController {
    ...
    public String editPage(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id == null) {
            model.addAttribute("entity", new Location());
        } else {
            model.addAttribute("entity", service.read(id));
        }
    ...
    @PostMapping
    public String save(@ModelAttribute Location entity, RedirectAttributes ra) {
        if (entity.getId() == null) {
            try {
                Location createdEntity = service.create(entity);
