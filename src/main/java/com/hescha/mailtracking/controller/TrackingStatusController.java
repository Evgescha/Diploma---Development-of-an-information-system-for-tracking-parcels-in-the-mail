...
@RequestMapping(TrackingStatusController.CURRENT_ADDRESS)
public class TrackingStatusController {
    public static final String API_PATH = "trackingstatus";
    ...
    public String editPage(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id == null) {
            model.addAttribute("entity", new TrackingStatus());
        } else {
            model.addAttribute("entity", service.read(id));
        }

        model.addAttribute("parcelStatus_list", ParcelStatus.values());
    ...
    @PostMapping
    public String save(@ModelAttribute TrackingStatus entity, RedirectAttributes ra) {
        if (entity.getId() == null) {
            try {
                TrackingStatus createdEntity = service.create(entity);
